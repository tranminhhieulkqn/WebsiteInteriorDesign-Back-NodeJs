var mongoose = require('mongoose');

/**
 * Define table LikesDetails in database.
 */
var LikesDetailsSchema = new mongoose.Schema({
    createdDate: { type: Date, required: true, default: Date.now },
    postID: { type: String, required: true },
    authorID: { type: String, required: true }
});

/**
 * Sample json to create LikesDetails
    Method: POST
    {
        "createdDate": Date.now, // auto
        "postID": "idofpost"
        "authorID": "6082815db8de82110c0bd714",
        "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..95CAg8g1BaoiCoVQ873xTieBqdqZcOelJ8Mv_mGW5SQ"
    }
 */

/**
 * Make this available to our likesDetails in our Node applications
 */
module.exports = mongoose.model('LikesDetails', LikesDetailsSchema);